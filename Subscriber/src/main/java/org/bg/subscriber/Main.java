...
public class Main {
    ...
    private static final int MAX_CHANNEL_ID = 100000;
    private static final int EXPECTED_SIZE_FOR_CLIENT = 20000;
    private static final int NUMBER_OF_CLIENTS_TO_TEST = 50;
    private static final Logger LOGGER = LoggerFactory.getLogger(Main.class);
    private static Random random = new Random();
    public static void main(String[] args) {
        EventsNotifier notifier = new EventsNotifier();
        HashSet<SubscribedClient> clients = new HashSet<>();


        for (int i = 1; i <= NUMBER_OF_CLIENTS_TO_TEST; i++){
            clients.add(new SubscribedClient(notifier, String.valueOf(i)));
        }

        LOGGER.debug("Clients: {}", clients);
        clients.forEach(client -> getChannelIds().forEach(client::subscribeToData));

        LOGGER.info("Subscription ended, current state: {}", clients);

        while (true) {
            notifier.handleMsg(getChannelName(getRandomNumberUsingNextInt()), "akljdasikodsakdmsakdlsakdnsakdnakdnjkdfsjlfnd");
        }
